...
void MFCC::GetFeature(short *RdBuf, float *Cep, int len)//cep¶à´ó£¿£¿£¿
{
	int j;
	float data[Frame];//double?? Frame??
	for (j = 1; j<len / 2; j++)
		data[j] = (float)RdBuf[j] - 0.93*(float)RdBuf[j - 1];

	for (j = len / 2; j<Frame; j++)
		data[j] = 0.0;
	data[0] = (float)RdBuf[0];
	MultiHamming(data);
	GetVector(data, Cep);
}


void MFCC::MultiHamming(float *data)
{
	double twopi;
